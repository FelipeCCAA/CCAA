...
def transicionar_orden(*, orden_id, estado_nuevo, usuario, motivo=""):
    # `of=("self",)` acota el bloqueo a la orden. Sin eso PostgreSQL rechaza
    # la consulta —«FOR UPDATE no puede ser aplicado al lado nulable de un
    # outer join»—: `plan` es nulable, así que `select_related` genera un LEFT
    # JOIN, y una orden correctiva no tiene plan que bloquear.
    orden = (
        OrdenTrabajo.objects.select_for_update(of=("self",))
        .select_related("plan")
        .get(pk=orden_id)
    )
