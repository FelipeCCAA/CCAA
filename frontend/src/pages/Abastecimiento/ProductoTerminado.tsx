...
export default function ProductoTerminado() {
  ...
  return <div className="space-y-6">
    ...
    <Tarjeta titulo="Despachos" descripcion="Calidad se vuelve a comprobar al autorizar y al ejecutar.">
      <div className="space-y-3">{(despachos.datos ?? []).map((d) => <div key={d.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-4"><div><div className="font-semibold text-slate-800">{d.numero} · {d.cliente_nombre}</div><div className="text-sm text-slate-500">{d.detalles.length} pallet(s) · {d.detalles.reduce((s, x) => s + Number(x.kg_neto), 0).toLocaleString("es-CL")} kg</div></div><div className="flex items-center gap-2"><Estado valor={d.estado}/>{d.estado === "borrador" && <button className={claseBoton} onClick={() => cambiar(d.id, "autorizar")}>Autorizar</button>}{d.estado === "autorizado" && <button className={claseBoton} onClick={() => cambiar(d.id, "ejecutar")}>Confirmar salida</button>}</div></div>)}</div>
    </Tarjeta>
  </div>;
}
