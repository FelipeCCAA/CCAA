  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/30">
      ...
      <p className="mt-4 text-3xl font-semibold tracking-tight text-slate-950">
        {valor}{unidad && <span className="ml-1 text-base font-medium text-slate-400">{unidad}</span>}
      </p>
      ...
      <div className="mx-auto max-w-[1500px] space-y-7">
        ...
        <section className="grid gap-5 xl:grid-cols-[1.4fr_0.8fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Planificación vs realidad</p><h2 className="mt-1 text-lg font-semibold text-slate-900">Resultado de la semana operacional</h2></div><Link to="/planificacion" className="inline-flex items-center gap-1 text-sm font-semibold text-emerald-700 hover:text-emerald-800">Ver planificación <ChevronRight className="h-4 w-4" /></Link></div>
            <div className="mt-6 grid gap-5 sm:grid-cols-3">
              {[["Planificado", recibida?.plan, "L"], ["Recibido", recibida?.real, "L"], ["Procesado", datos.contraste?.resumen.leche_consumida.real, "L"]].map(([etiqueta, valor, unidad]) => <div key={String(etiqueta)}><p className="text-sm text-slate-500">{etiqueta}</p><p className="mt-1 text-2xl font-semibold tabular-nums text-slate-900">{typeof valor === "number" ? numero.format(valor) : "—"} <span className="text-sm font-medium text-slate-400">{unidad}</span></p></div>)}
            </div>
            <div className="mt-6 rounded-xl bg-slate-50 p-4"><div className="flex items-center justify-between text-sm"><span className="font-medium text-slate-600">Cumplimiento de recepción</span><strong className="text-slate-900">{cumplimiento == null ? "—" : `${porcentaje.format(cumplimiento)}%`}</strong></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${Math.max(0, Math.min(100, cumplimiento ?? 0))}%` }} /></div><p className="mt-3 text-xs text-slate-500">Los datos reales provienen de descargas y movimientos; nunca reemplazan los valores planificados.</p></div>
          </article>

          <article className="rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white sm:p-6">
            <div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-emerald-300">Puertas de control</p><h2 className="mt-1 text-lg font-semibold">Atención requerida</h2></div><Gauge className="h-6 w-6 text-slate-500" /></div>
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 border-t border-white/10 pt-5 text-center"><div><p className="text-xl font-semibold">{pendientesCalidad.length}</p><p className="mt-1 text-[10px] uppercase tracking-wide text-slate-500">Calidad</p></div><div><p className="text-xl font-semibold">{datos.recepcion?.por_estado.retenida ?? 0}</p><p className="mt-1 text-[10px] uppercase tracking-wide text-slate-500">Retenidas</p></div><div><p className="text-xl font-semibold">{datos.mantenimiento?.ordenes_abiertas ?? 0}</p><p className="mt-1 text-[10px] uppercase tracking-wide text-slate-500">Mantención</p></div></div>
          </article>
        </section>

        <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
            <div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Inventario de proceso</p><h2 className="mt-1 text-lg font-semibold text-slate-900">Ocupación de silos</h2></div><Link to="/leche/silos" className="text-sm font-semibold text-emerald-700">Ver silos</Link></div>
            <div className="mt-5 space-y-4">{(datos.ocupacion?.silos ?? []).slice(0, 6).map((silo) => <div key={silo.silo_id}><div className="flex items-center justify-between text-sm"><span className="font-medium text-slate-700">{silo.codigo}</span><span className={silo.excedido || silo.negativo ? "font-semibold text-rose-600" : "text-slate-500"}>{numero.format(silo.litros)} / {numero.format(silo.capacidad)} L · {porcentaje.format(silo.pct)}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${silo.excedido || silo.negativo ? "bg-rose-500" : silo.pct >= 85 ? "bg-amber-500" : "bg-sky-500"}`} style={{ width: `${Math.max(0, Math.min(100, silo.pct))}%` }} /></div></div>)}{!cargando && !datos.ocupacion?.silos.length && <p className="py-8 text-center text-sm text-slate-400">No hay silos con movimientos registrados.</p>}</div>
          </article>

          <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-5 sm:px-6"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Trazabilidad reciente</p><h2 className="mt-1 text-lg font-semibold text-slate-900">Últimos lotes</h2></div><Link to="/produccion" className="text-sm font-semibold text-emerald-700">Ver producción</Link></div>
            <div className="divide-y divide-slate-100">{datos.lotes.map((lote) => <Link key={lote.id} to="/produccion" className="grid gap-2 px-5 py-4 transition hover:bg-slate-50 sm:grid-cols-[1.1fr_1fr_auto] sm:items-center sm:px-6"><div><p className="font-semibold text-slate-800">{lote.codigo_lote}</p><p className="mt-0.5 text-xs text-slate-400">{lote.op || "Sin orden"} · {lote.fecha}</p></div><div><p className="text-sm text-slate-600">{lote.producto_nombre}</p><p className="mt-0.5 text-xs text-slate-400">{lote.equipo_nombre || lote.linea || "Equipo por asignar"}</p></div><span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{lote.estado_etiqueta}</span></Link>)}{!cargando && datos.lotes.length === 0 && <p className="px-6 py-10 text-center text-sm text-slate-400">Todavía no hay lotes registrados.</p>}</div>
          </article>
        </section>

        <footer className="flex flex-wrap gap-3 border-t border-slate-200 pt-5 text-xs text-slate-500"><span className="inline-flex items-center gap-1.5"><Truck className="h-3.5 w-3.5" /> Recepción real</span><span className="inline-flex items-center gap-1.5"><Droplets className="h-3.5 w-3.5" /> Saldos por movimientos</span><span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5" /> Calidad transversal</span><span className="inline-flex items-center gap-1.5"><Wrench className="h-3.5 w-3.5" /> Disponibilidad operacional</span></footer>
      </div>
    </main>
  );
}
