...
export default function Mantenimiento() {
  ...
  return <main className="px-6 py-8 lg:px-10"><div className="mx-auto max-w-7xl space-y-8">
    <header><p className="text-sm font-semibold uppercase tracking-wider text-green-700">Activos</p><h1 className="mt-2 text-3xl font-bold text-slate-900">Mantenimiento</h1><p className="mt-2 text-slate-600">Planes preventivos, fallas, órdenes de trabajo y disponibilidad de equipos.</p></header>
    {error && <ErrorState mensaje={error} />}
    {cargando ? <PageLoader /> : <>
      <section className="grid gap-4 md:grid-cols-3">
        {indicadores.map(({ titulo, valor, icono: Icono }) => <article key={titulo} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><p className="text-sm text-slate-600">{titulo}</p><p className="mt-2 text-3xl font-bold text-slate-900">{valor}</p></div><Icono className="h-6 w-6 text-green-700" /></div></article>)}
      </section>
