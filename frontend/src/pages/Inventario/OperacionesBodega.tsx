      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        {operacion === "despacho" && <>
          ...
          {datos.tipo_despacho === "pallet" ? (
            <select required className={campo} value={datos.pallet} onChange={(e) => setDatos({ ...datos, pallet: e.target.value })}><option value="">Unidad logística disponible…</option>{productos.filter((p) => p.estado_inventario === "disponible" && p.ubicacion_tipo === "disponible").map((p) => <option key={p.id} value={p.pallet}>{p.tipo_unidad_logistica_etiqueta} · {p.pallet_codigo} · {p.producto_nombre} · {p.kg_neto} kg</option>)}</select>
          ) : <>
