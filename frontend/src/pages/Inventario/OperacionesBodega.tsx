  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6">
    ...
    {operacion && <form onSubmit={guardar} className="mt-5 rounded-2xl bg-slate-50 p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {operacion === "entrada" && <><select required className={campo} value={datos.insumo} onChange={(e) => setDatos({ ...datos, insumo: e.target.value })}><option value="">Envase o material…</option>{insumos.map((i) => <option key={i.id} value={i.id}>{i.codigo} · {i.nombre} ({i.unidad})</option>)}</select><input required className={campo} placeholder="Lote del proveedor" value={datos.lote} onChange={(e) => setDatos({ ...datos, lote: e.target.value })} /><SelectorUbicacion ubicaciones={ubicaciones} valor={datos.ubicacion} alCambiar={(ubicacion) => setDatos({ ...datos, ubicacion })} /><input required min="0.001" step="0.001" type="number" className={campo} placeholder="Cantidad recibida" value={datos.cantidad} onChange={(e) => setDatos({ ...datos, cantidad: e.target.value })} /></>}
        ...
        {operacion === "despacho" && <><input required className={campo} placeholder="Nº de despacho" value={datos.numero} onChange={(e) => setDatos({ ...datos, numero: e.target.value })} /><select required className={campo} value={datos.cliente} onChange={(e) => setDatos({ ...datos, cliente: e.target.value })}><option value="">Cliente…</option>{clientes.filter((c) => c.activo).map((c) => <option key={c.id} value={c.id}>{c.codigo} · {c.nombre}</option>)}</select><select required className={campo} value={datos.pallet} onChange={(e) => setDatos({ ...datos, pallet: e.target.value })}><option value="">Pallet disponible…</option>{productos.filter((p) => p.estado_inventario === "disponible" && p.ubicacion_tipo === "disponible").map((p) => <option key={p.id} value={p.pallet}>{p.pallet_codigo} · {p.producto_nombre} · {p.kg_neto} kg</option>)}</select><input className={campo} placeholder="Observación" value={datos.motivo} onChange={(e) => setDatos({ ...datos, motivo: e.target.value })} /></>}
      </div>
