      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-8 grid gap-5 md:grid-cols-3">
          {[
            ...
            { etiqueta: "Bajo punto de reposición", valor: insumos.filter((i) => Number(i.stock_disponible) <= Number(i.punto_reposicion)).length, Icono: PackageOpen },
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          ...
          {error ? <p className="p-6 text-red-700">{error}</p> : insumos.length === 0 ? (
            ...
            <div className="overflow-x-auto"><table className="w-full text-left text-sm">
              ...
              <tbody>{insumos.map((i) => <tr key={i.id} className="border-t border-slate-100"><td className="px-6 py-4 font-medium text-slate-800">{i.codigo} · {i.nombre}</td><td className="px-6 py-4 text-slate-600">{i.area_etiqueta}</td><td className="px-6 py-4">{numero(String(i.stock_disponible))} {i.unidad}</td><td className="px-6 py-4">{numero(i.punto_reposicion)} {i.unidad}</td><td className="px-6 py-4">{numero(i.eoq)} {i.unidad}</td></tr>)}</tbody>
            </table></div>
          )}
        </section>
