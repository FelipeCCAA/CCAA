...
const errorDe = (e: unknown) => {
  ...
  return datos && typeof datos === "object" ? Object.values(datos as Record<string, unknown>).flat().map(String).join(" ") : "No se pudo crear la corrida.";
};
...
export default function NuevaCondensacion({ onCerrar, onCreada }: { onCerrar: () => void; onCreada: (corrida: CorridaCondensacion) => void }) {
  ...
  const guardar = async (e: React.FormEvent) => { e.preventDefault(); setGuardando(true); setError(""); try { onCreada(await crearCondensacionGuiada({ lote: Number(lote), silo_destino: Number(destino) })); } catch (err) { setError(errorDe(err)); } finally { setGuardando(false); } };
