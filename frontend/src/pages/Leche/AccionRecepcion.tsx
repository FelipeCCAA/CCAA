...
import { AlertTriangle, FlaskConical, TestTube2, Warehouse, X } from "lucide-react";
...
function AccionRecepcion({ accion, recepcion, silos, responsables, alCerrar, alGuardar }: Props) {
  const muestraSugerida = (recepcion.guia || `REC-${recepcion.id}`)
    .replace(/\s+/g, "-")
    .toUpperCase();
        ...
        <form onSubmit={guardar}>
          <div className="p-6">
            <div className="mb-6 grid grid-cols-2 gap-3 rounded-2xl bg-slate-50 p-4 text-sm sm:grid-cols-4">
              ...
              <div><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-600">Módulos</p><p className="mt-1 font-semibold text-slate-800">{recepcion.modulos.length > 0 ? recepcion.modulos.map((m) => `M${m.numero}`).join(", ") : "—"}</p></div>
            ...
            {accion === "muestra" && <div className="space-y-4">
              ...
              <p className="rounded-xl bg-blue-50 px-4 py-3 text-xs leading-5 text-blue-700">La muestra identifica el camión completo; la crioscopía de cada módulo ya quedó registrada al recibirlo.</p>
            </div>}
