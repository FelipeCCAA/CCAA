    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-3 backdrop-blur-[2px] sm:p-6">
      <div className="mx-auto w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl shadow-slate-950/20">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
          ...
          <button type="button" onClick={alCerrar} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100" aria-label="Cerrar"><X className="h-5 w-5" /></button>
        </div>

        <form onSubmit={guardar}>
          <div className="p-6">
            <div className="mb-6 grid grid-cols-2 gap-3 rounded-2xl bg-slate-50 p-4 text-sm sm:grid-cols-4">
              <div><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Guía</p><p className="mt-1 font-semibold text-slate-800">{recepcion.guia || "—"}</p></div>
              <div><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Módulo</p><p className="mt-1 font-semibold text-slate-800">{recepcion.modulo || "—"}</p></div>
              <div><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Tipo</p><p className="mt-1 font-semibold text-slate-800">{recepcion.tipo_leche}</p></div>
