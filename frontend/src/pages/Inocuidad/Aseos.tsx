...
function FormularioPlan({ catalogos, equipos, silos, alCerrar, alCrear }: {
  ...
  return <form onSubmit={enviar} className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5">
    <div className="mb-5 flex items-start justify-between"><div><h2 className="font-semibold text-slate-900">Programar aseo</h2><p className="mt-1 text-sm text-slate-500">Define dónde se hará. El registro queda en la planilla del área.</p></div><button type="button" onClick={alCerrar} className="rounded-lg p-2 text-slate-400 hover:bg-white"><X className="h-5 w-5" /></button></div>
...
function ControlAseo({ aseo, catalogos, alCambiar, alCerrar }: { aseo: AseoCip; catalogos: CatalogosAseo; alCambiar: (a: AseoCip) => void; alCerrar: () => void }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5">
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Control de aseo #{aseo.id}</p><h2 className="mt-1 text-xl font-bold text-slate-900">{aseo.objetivo_nombre}</h2><p className="mt-1 text-sm text-slate-500">{aseo.area_etiqueta} · {aseo.tipo_aseo_etiqueta} · registrado por {aseo.responsable_nombre}</p>{aseo.ejecutado_por_nombre && <p className="mt-1 text-xs text-slate-400">Ejecutado por {aseo.ejecutado_por_nombre}{aseo.verificado_por_nombre ? ` · verificado por ${aseo.verificado_por_nombre}` : ""}</p>}</div><button type="button" onClick={alCerrar} className="rounded-lg p-2 text-slate-400 hover:bg-slate-50"><X className="h-5 w-5" /></button></div>
...
export default function Aseos() {
  ...
  return <main className="px-6 py-8 lg:px-10"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {cargando || !catalogos ? <PageLoader /> : <>
      ...
      ].map(([etiqueta, valor, Icono]) => { const I = Icono as typeof Beaker; return <article key={String(etiqueta)} className="rounded-2xl border border-slate-200 bg-white p-5"><I className="h-5 w-5 text-emerald-700" /><p className="mt-3 text-sm text-slate-500">{String(etiqueta)}</p><p className="mt-1 text-2xl font-bold text-slate-900">{String(valor)}</p></article>; })}</section>
