  ...
  const contenido = (
    <>
      <div className="flex h-20 items-center justify-between border-b border-slate-100 px-5">
        ...
        <button type="button" onClick={() => setAbierto(false)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100 md:hidden" aria-label="Cerrar menú"><X className="h-5 w-5" /></button>
      </div>
        ...
        <div className="space-y-6">
          {grupos.map((grupo) => (
            <section key={grupo.etiqueta}>
              <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-600">{grupo.etiqueta}</p>
                ...
                {grupo.modulos.map((modulo) => {
                  ...
                  return (
                    ...
                    >
                      {({ isActive }) => (
                        <>
                          <Icono className={`h-[18px] w-[18px] shrink-0 ${isActive ? "text-emerald-700" : "text-slate-600 group-hover:text-slate-600"}`} strokeWidth={1.8} />
      ...
      <div className="border-t border-slate-100 p-3">
        <div className="mb-2 flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-3">
          ...
          <div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800">{sesion ? nombreParaMostrar(sesion.usuario) : "Sin sesión"}</p><p className="truncate text-[11px] text-slate-600">{sesion ? cargoParaMostrar(sesion.usuario) : ""}</p></div>
        </div>
        <button type="button" onClick={salir} className={`${enlaceBase} w-full text-slate-600 hover:bg-rose-50 hover:text-rose-700`}><LogOut className="h-[18px] w-[18px]" strokeWidth={1.8} />Cerrar sesión</button>
      </div>
    </>
  );
